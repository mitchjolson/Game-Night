...
router.get('/:id', (req, res) => {
    ...
    const sqlText = `(select username, friends.user2 as friend_ID from "user" join friends on friends.user2 = "user".id where friends.user1 = $1 and status = 'accepted') union (select username, friends.user1 as friend_ID from "user" join friends on friends.user1 = "user".id where friends.user2 = $1 and status = 'accepted');`
